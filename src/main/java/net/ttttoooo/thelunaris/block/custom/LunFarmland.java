...
import net.ttttoooo.thelunaris.util.BushblockDefiner;

public class LunFarmland extends FarmBlock {
    public LunFarmland(Properties properties) {
        super(properties);
    }
    ...
    public BlockState getStateForPlacement(BlockPlaceContext context) {
        return !this.defaultBlockState().canSurvive(context.getLevel(), context.getClickedPos()) ? ModBlocks.LUNDIRT.get().defaultBlockState() : this.defaultBlockState();
    }
    ...
    public void fallOn(Level level, BlockState state, BlockPos pos, Entity entity, float fallDistance) {
        if (!level.isClientSide() && ForgeHooks.onFarmlandTrample(level, pos, ModBlocks.LUNDIRT.get().defaultBlockState(), fallDistance, entity)) { // Forge: Move logic to Entity#canTrample
            turnToDirt(state, level, pos);
        }
    ...
    public boolean canSustainPlant(BlockState state, BlockGetter level, BlockPos pos, Direction direction, IPlantable plantable) {
        ...
        return (plantable instanceof BushBlock bushBlock && ((BushblockDefiner) bushBlock).callMayPlaceOn(state, level, pos)) || PlantType.CROP.equals(type) || PlantType.PLAINS.equals(type);
    }
