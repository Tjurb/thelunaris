...
public class ModPortalFrame {
    ...
    public static Optional<ModPortalFrame> findPortalShape(LevelAccessor level, BlockPos bottomLeft, Predicate<ModPortalFrame> predicate, Direction.Axis axis) {
        Optional<ModPortalFrame> optional = Optional.of(new ModPortalFrame(level, bottomLeft, axis)).filter(predicate);
        if (optional.isPresent()) {
            return optional;
        } else {
            Direction.Axis directionAxis = axis == Direction.Axis.X ? Direction.Axis.Z : Direction.Axis.X;
            return Optional.of(new ModPortalFrame(level, bottomLeft, directionAxis)).filter(predicate);
        }
    }

    public ModPortalFrame(LevelAccessor level, BlockPos bottomLeft, Direction.Axis axis) {
        this.level = level;
        this.axis = axis;
